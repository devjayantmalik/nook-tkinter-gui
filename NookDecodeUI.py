...
class NookDecodeUI:
    def __init__(self, parent: ThemedTk):
        ...
        self.btn_decode = ttk.Button(self.frame, text='Decode', command=self.none)
        self.btn_decode.grid(column=3, row=2, sticky=W)

        # create encode key label and text area
        ttk.Label(self.frame, text='Enter Decode Key').grid(column=2, row=3, sticky=W)
        ...
        ttk.Label(self.frame, text='Select Decode Algorithm').grid(column=2, row=5, sticky=W)
        self.btn_lsb = ttk.Button(self.frame, text='LSB',  command=self.none)
        self.btn_lsb.grid(column=2, row=6, sticky=W)

        ttk.Label(self.frame, text='Decode Message').grid(column=2, row=7, sticky=W)
        ttk.Label(self.frame, text='You will see result in this area, once algorithm is selected.').grid(column=2, row=8, columnspan=3, sticky=W)
...
if __name__ == '__main__':
    ...
    win.title("Nook Decode GUI - Shaurya Bhatnagar and Jayant Malik")
    NookDecodeUI(parent=win)
    win.minsize(600, 400)
    win.maxsize(600, 600)
